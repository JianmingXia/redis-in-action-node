...
async function run() {
    ...
    console.log(`Let's drop the maximum number of cookies to 0 to clean them out`);
    ...
    console.log(`To test that we've cached the request, we'll pass a bad callback`);
    const result2 = await ch02.cacheRequest(redis, url, callback);
    console.log(`We ended up getting the same response: ${result2}`);

    const result3 = await ch02.cacheRequest(redis, 'http://test.com/?item=itemX&_=1234536', callback);
